...
// ParsedRule represents a parsed validation rule with its name and parameters
...
// ParseRules parses a rule string into a slice of ParsedRule
...
// SplitRules splits a rule string by pipe character, respecting escaped pipes
...
// parseConditionalRules parses conditional rules like "required_if:field,value"
// This function is currently unused but kept for future implementation
// of more advanced conditional validation features.
/*
func parseConditionalRules(rule ParsedRule) *ConditionalRule {
	// Handle Laravel-style conditional rules
	conditionalRuleTypes := map[string]bool{
		"required_if":      true,
	...
	if !conditionalRuleTypes[rule.Name] || len(rule.Params) < 2 {
		return nil
	}
	...
	return &ConditionalRule{
		Field:    rule.Params[0],
		Value:    rule.Params[1],
		Operator: "=", // Default operator is equals
		Rules:    []ParsedRule{{Name: "required"}},
	}
}
*/
