...
func TestDebugParseRules(_ *testing.T) {
	inputs := []string{
		"required|alpha",
	...
	for _, input := range inputs {
		...
		fmt.Printf("Manual split result: %#v\n", manualParts)
		fmt.Printf("Manual split number of parts: %d\n", len(manualParts))

		// Debug SplitRules function
		parts := SplitRules(input)
		fmt.Printf("SplitRules result: %#v\n", parts)
		fmt.Printf("Number of parts: %d\n", len(parts))

		// Debug ParseRules function
		result := ParseRules(input)
		fmt.Printf("ParseRules result: %+v\n", result)

		// Print each parsed rule
		for i, rule := range result {
			fmt.Printf("Rule %d: Name=%q, Params=%v\n", i, rule.Name, rule.Params)
		}
	}
}
